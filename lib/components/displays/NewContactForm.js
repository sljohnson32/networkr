...
export default class NewContactForm extends Component {
  ...
  editSubmitButton(addNewContact, editContact, setSelectedContact) {
    if(this.state.existingContact && !this.state.editView ) {
      ...
      return (
        <button className = 'save-changes'
          onClick = { () => this.submitContact(editContact, setSelectedContact) }
        >SAVE CHANGES
        </button>
      )
    } else if (!this.state.existingContact && this.state.editView) {
        return (
          <button className ='submit-contact'
            onClick = { () => this.submitContact(addNewContact, setSelectedContact) }
          >SUBMIT
          </button>
        )
    }
  ...
  contactDisplayHandler(addNewContact, editContact, viewDetails) {
  ...
  render() {
    ...
    return (
      this.contactDisplayHandler(addNewContact, editContact, setSelectedContact)
    )
  }
}
